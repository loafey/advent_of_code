...
for year in "${YEARS[@]}"; do
    ...
    for day in $(seq 1 25); do
        ...
        part1=$(systemd-run --user -q --scope -p MemoryLimit=4000M -p RuntimeMaxSec=2 ./target/release/advent_of_code -s $year $day 1)
        part2=$(systemd-run --user -q --scope -p MemoryLimit=4000M -p RuntimeMaxSec=2 ./target/release/advent_of_code -s $year $day 2)
        if [ -n "$part1" ]; then
